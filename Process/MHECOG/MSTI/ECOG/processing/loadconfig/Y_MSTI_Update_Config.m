...
S1_S2 = cellfun(@(x) string(strsplit(x, ",")), string(strsplit(mProtocol(pIndex).S1_S2, ";")), "uni", false)';
...
orderIndex = cellfun(@(x) str2double(string(strsplit(x, "-"))), strsplit(convertCharsToStrings(mProtocol(pIndex).orderIndex), ';')', "uni", false);
