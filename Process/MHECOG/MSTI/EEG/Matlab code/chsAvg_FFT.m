...
scaleAxes("x", [min(TargetFreq) - 10, max(TargetFreq) + 10]);
scaleAxes("y", [0, 0.5]);
lines = [];
lines(1).X = TargetFreq(1); lines(1).color = "k";
lines(2).X = TargetFreq(2); lines(2).color = "k"; 
addLines2Axes(lines);
FigchsAvg_FFT_click = gcf;
print(FigchsAvg_FFT_click, fullfile(FIGPATH, strcat(plotstr, "-FFT_click")), "-djpeg", "-r200");
...
lines(2).X = TargetFreq(2); lines(2).color = "k"; 
addLines2Axes(lines);
FigAudichsAvg_FFT_click = gcf;
print(FigAudichsAvg_FFT_click, fullfile(FIGPATH, strcat("audi", plotstr, "-FFT_click")), "-djpeg", "-r200");
