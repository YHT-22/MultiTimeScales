...
%     ft_setPath2Top;
    ...
    matFileNames = matFileNames(cellfun(@(x) ismember(str2double(obtainArgoutN(@fileparts, 2, x)), 1:600), matFileNames));
    folders = dir(ROOTPATH);
    folders = folders([folders.isdir]);
    folders = folders(3:end);

    for idx = 1:length(matFileNames)
        load(fullfile(ROOTPATH, matFileNames{idx}), "-mat");
%         BDFPATH = fullfile(ROOTPATH, folders(strcmpi({folders.name}, protocol)).name);
        if exist("rules", "var")
            protocol = unique(string(rules.protocol));
        end
        ...
        %%%%%%%%%%%%%%% checkEEGEvent %%%%%%%%%%%%%%%
        EEG.event(isnan(double(string({EEG.event.type}')))) = [];
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if exist("rules", "var")
            trialDatasets(idx).trialAll = Y_EEGBehaviorProcess(trialsData, EEGDatasets(idx), rules);
        else
            trialDatasets(idx).trialAll = Y_EEGBehaviorProcess(trialsData, EEGDatasets(idx));            
        end
    end
